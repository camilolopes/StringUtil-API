package com.string.util;

import java.util.ArrayList;
import java.util.List;

/**
 * @author camilolopes
 *
 */
public  final class StringUtil {
	private StringUtil(){		
	}
	/**
	 * 
	 * @param value
	 * @return a String without character duplicated
	 */
	public static String removeDuplicateCharacter(String value) {
		if(value==null){
		throw new IllegalArgumentException("parameter cannot be null");	
		}
		String result = value.substring(0, 1);
		List<Character> listValidCharactereRoute = removeDuplicate(value);
		for (Character character : listValidCharactereRoute) {
			result += character.toString();
		}
		return result;
	}
	
	private static List<Character> removeDuplicate(String value){
		CharSequence charValue = value;		
		List<Character> valueRoute = new ArrayList<Character>();
		int positionIndex = 1;
		for (int i = positionIndex; i < value.length(); i++) {		
			 if(i>=positionIndex && charValue.charAt(i)!= charValue.charAt(i-positionIndex)){
				valueRoute.add(charValue.charAt(i));
			}
		}		
		return valueRoute;
	}
}